...
use Joinca\ZKUploader\Acl\User\SessionRoleContext;
use Joinca\ZKUploader\Authentication\CallableAuthentication;
use Joinca\ZKUploader\Backend\BackendFactory;
use Joinca\ZKUploader\Cache\CacheManager;
use Joinca\ZKUploader\Cache\Adapter\BackendAdapter;
...
use Joinca\ZKUploader\Exception\InvalidCsrfTokenException;
use Joinca\ZKUploader\Exception\InvalidPluginException;
use Joinca\ZKUploader\Filesystem\Folder\WorkingFolder;
use Joinca\ZKUploader\Filesystem\Path;
use Joinca\ZKUploader\Operation\OperationManager;
use Joinca\ZKUploader\Plugin\PluginInterface;
use Joinca\ZKUploader\Request\Transformer\JsonTransformer;
...
use Joinca\ZKUploader\Security\Csrf\DoubleSubmitCookieTokenValidator;
 ...
 * 
 */
class ZKUploader extends Container implements HttpKernelInterface
{
    const VERSION = '3.3.0';

    const COMMANDS_NAMESPACE = 'Joinca\\ZKUploader\\Command\\';
    const PLUGINS_NAMESPACE = 'Joinca\\ZKUploader\\Plugin\\';
    ...
    public function __construct($config)
    {
        ...
        $this['resolver'] = function () use ($app) {
            ...
            $commandResolver->setCommandsNamespace(ZKUploader::COMMANDS_NAMESPACE);
            $commandResolver->setPluginsNamespace(ZKUploader::PLUGINS_NAMESPACE);
    ...
    {
        /* @var $authentication \Joinca\ZKUploader\Authentication\AuthenticationInterface */
        $authentication = $this['authentication'];

        if (!$authentication->authenticate()) {
            ini_set('display_errors', 0);
            throw new ZKUploaderException('ZKUploader is disabled', Error::CONNECTOR_DISABLED);
        }
    }
    ...
    public function checkCsrfToken(Request $request)
    {
        ...
        /* @var $csrfTokenValidator \Joinca\ZKUploader\Security\Csrf\TokenValidatorInterface */
    ...
    public function createResponse(GetResponseForControllerResultEvent $event)
    {
        ...
        $eventName = ZKUploaderEvent::CREATE_RESPONSE_PREFIX . lcfirst($commandName);
    ...
    public function afterCommand(FilterResponseEvent $event)
    {
        ...
        $commandName = $event->getRequest()->get('command');
        $eventName = ZKUploaderEvent::AFTER_COMMAND_PREFIX . lcfirst($commandName);
    ...
    /**
     * Main method used to handle a request by ZKUploader.
     *
    ...
    protected function registerPlugins()
    {
        ...
        foreach ($pluginsEntries as $pluginInfo) {
            ...
            $pluginClassName = ZKUploader::PLUGINS_NAMESPACE . $pluginName . '\\' . $pluginName;

            if (!class_exists($pluginClassName)) {
                throw new InvalidPluginException(sprintf('ZKUploader plugin "%s" not found (%s)', $pluginName, $pluginClassName), array('pluginName' => $pluginName));
            }
            ...
            if ($pluginObject instanceof PluginInterface) {
                ...
                throw new InvalidPluginException(sprintf('ZKUploader plugin class must implement %sPluginInterface', ZKUploader::PLUGINS_NAMESPACE), array('pluginName' => $pluginName));
            }
        }
    }
    ...
    {
        $errorMessage = 'The PHP installation does not meet the minimum system requirements for ZKUploader. %s Please refer to ZKUploader documentation for more details.';

        if (version_compare(PHP_VERSION, '5.4.0') < 0) {
            throw new ZKUploaderException(sprintf($errorMessage, 'Your PHP version is too old. ZKUploader 3.x requires PHP 5.4+.'), Error::CUSTOM_ERROR);
        }
        ...
        if (!empty($missingExtensions)) {
            throw new ZKUploaderException(sprintf($errorMessage, 'Missing PHP extensions: ' . implode(', ', $missingExtensions) . '.'), Error::CUSTOM_ERROR);
        }
    }
    ...
    public function boot(Request $request)
    {
        ...
        if ($config->get('debug') && $config->isDebugLoggerEnabled('zkuploader_log')) {
    ...
    public function registerStreamLogger()
    {
        ...
        /* @var $logsBackend \Joinca\ZKUploader\Backend\Backend */
    ...
    public function handle(Request $request, $type = HttpKernelInterface::MASTER_REQUEST, $catch = true)
    {
        ...
        /* @var $requestTransformer \Joinca\ZKUploader\Request\Transformer\TransformerInterface */
