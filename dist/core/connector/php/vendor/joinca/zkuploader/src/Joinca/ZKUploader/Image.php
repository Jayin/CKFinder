...
class Image
{
     ...
     *
     * @throws ZKUploaderException in case the image could not be initialized properly.
     */

    public function __construct($imageData, $bmpSupport = false)
    {
        if (!extension_loaded('gd')) {
            throw new ZKUploaderException('PHP GD library not found');
        }

        $imgInfo = @getimagesizefromstring($imageData);

        if ($imgInfo === false) {
            throw new ZKUploaderException('Unsupported image type');
        }
        ...
        if (!array_key_exists($this->mime, $supportedFormats) || !$supportedFormats[$this->mime]) {
            throw new ZKUploaderException('Unsupported image type: ' . $this->mime);
        }
        ...
        if (!is_resource($this->gdImage)) {
            throw new ZKUploaderException('Unsupported image type (not resource): ' . $this->mime);
        }
