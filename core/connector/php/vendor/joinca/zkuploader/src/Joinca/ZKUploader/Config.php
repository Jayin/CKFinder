...
use Joinca\ZKUploader\Exception\InvalidConfigException;
use Joinca\ZKUploader\Exception\InvalidResourceTypeException;
...
class Config
{
     ...
     *
     * @param array $options options passed to ZKUploader
     *
     * @return array
     */
    protected function mergeDefaultOptions($options)
    {
        $defaults = array(
            ...
            'roleSessionVar' => 'ZKUploader_UserRole',
            'accessControl' => array(
                array(
                    'role'          => '*',
                    'resourceType'  => '*',
            ...
            'plugins'                  => array(),
            'debugLoggers'            => array('zkuploader_log', 'error_log', 'firephp'),
    ...
    protected function validate()
    {
        $checkMissingNodes = function (array $required, array $actual, $prefix = '') {
            ...
            if (!empty($missing)) {
                throw new InvalidConfigException(sprintf(
                    "ZKUploader configuration doesn't contain all required fields. " .
        ...
        if (!is_callable($this->options['authentication'])) {
            throw new InvalidConfigException("ZKUploader Authentication config field must be a PHP callable");
        }

        if (!is_writable($this->options['tempDirectory'])) {
            throw new InvalidConfigException("The temporary folder is not writable for ZKUploader");
        }
    }
