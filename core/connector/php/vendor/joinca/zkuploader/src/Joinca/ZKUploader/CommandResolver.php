...
use Joinca\ZKUploader\Event\BeforeCommandEvent;
use Joinca\ZKUploader\Event\ZKUploaderEvent;
use Joinca\ZKUploader\Exception\InvalidCommandException;
use Joinca\ZKUploader\Exception\InvalidRequestException;
use Joinca\ZKUploader\Exception\MethodNotAllowedException;
 ...
 * 
 * The purpose of this class is to resolve which ZKUploader command should be executed
    ...
    public function getController(Request $request)
    {
        ...
        if (class_exists($commandClassName)) {
            ...
            if (!$reflectedClass->isInstantiable()) {
                throw new InvalidCommandException(sprintf('ZKUploader command class %s is not instantiable', $commandClassName));
            }
        ...
        if (null === $commandObject) {
            throw new InvalidCommandException(sprintf('ZKUploader command %s not found', $commandName));
        }

        if (!$commandObject instanceof CommandAbstract) {
            throw new InvalidCommandException(sprintf("ZKUploader command must be a subclass of CommandAbstract (%s given)", get_class($commandObject)));
        }

        if (!method_exists($commandObject, self::COMMAND_EXECUTE_METHOD)) {
            throw new InvalidCommandException(sprintf("ZKUploader command class %s doesn't contain required 'execute' method", $commandClassName));
        }

        if ($commandObject->getRequestMethod() !== $request->getMethod()) {
            throw new MethodNotAllowedException(sprintf('ZKUploader command %s expects to be called with %s HTTP request. Actual method: %s',
        ...
        $eventName = ZKUploaderEvent::BEFORE_COMMAND_PREFIX . lcfirst($commandName);
